...
class CatMlpPgModel(torch.nn.Module):
    ...
    def __init__(
        ...
        self.pi = torch.nn.Linear(self._output_size, output_size)
        self.value = torch.nn.Linear(self._output_size, 1)

    def forward(self, observation, prev_action, prev_reward):
        """ Compute the model on the input, assuming input shape [B,input_size].
            return the probability and values of actions
        """
        ...
        pi = F.softmax(self.pi(p), dim=-1)
        v = self.value(p).squeeze(-1)
